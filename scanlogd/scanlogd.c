...
/*  SIGNATURE 1 TRIGGER
    Packets following each other coming from same source address to different destination ports within short period of time. */
	...
	// If same source address is sending too many packets in this time frame then  log it
...
/*  SIGNATURE 2 TRIGGER
    Record header of the each packet to a file. Initiate analysis. */

	// Create file for storing received packets then do analysis on file

	// If same source address is sending many packets to different ports in ascending, descending or random order then log it
	
	// If same source address is sending packets to pre-defined most known ports then log it
 ...
 * up), so we use _exit(2) rather than exit(3) here
    ...
    // CHECK IF PROCESS IS RUNNING AS DAEMON, IF NOT FORK TO BECOME THEN EXIT
    ...
    if (!fork()){       // scanlogd process executed and it is not daemon (parent)         
        ...
        // continue;  	// act as daemon, initiate port scan detection
        //	}     				// scanlogd is running as daemon (child)
